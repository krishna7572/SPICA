...
class MainActivity : AppCompatActivity(), RecognitionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Commands button — START LISTENING ke upar
        findViewById<Button>(R.id.commandsBtn).setOnClickListener {
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    Toast.makeText(this, "Model load failed: ${e.message}",
    ...
    override fun onResult(hypothesis: String?) {
        ...
        if (text.contains("\"text\"")) {
            val extracted = text.substringAfter("\"text\" : \"")
    ...
    private fun processCommand(text: String) {
        if (Commands.SWITCH_TO_HINDI.any { text.contains(it) }) {
            currentLang = "hi"; stopListening(); startListening()
            Toast.makeText(this, "Hindi mode", Toast.LENGTH_SHORT).show(); return
        }
        if (Commands.SWITCH_TO_ENGLISH.any { text.contains(it) }) {
            currentLang = "en"; stopListening(); startListening()
            Toast.makeText(this, "English mode", Toast.LENGTH_SHORT).show(); return
        }
        when {
            Commands.startAudio(this).any { text.contains(it) } -> startAudio()
            Commands.stopAudio(this).any { text.contains(it) } -> stopAudio()
            Commands.startVideo(this).any { text.contains(it) } -> startVideo()
            Commands.stopVideo(this).any { text.contains(it) } -> stopVideo()
            Commands.emergency(this).any { text.contains(it) } -> triggerEmergency()
        }
    }
